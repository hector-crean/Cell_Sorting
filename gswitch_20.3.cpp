...
double Type[Ncells]; // this distinguishes type A=sender from type B=receiver

double HeterotropicBoundary[Ncells][9];
...
int main(int argc, char* argv[]){
    ...
    cout<< "Type: 1. name output 2. cell adherence (stiff cells) 3. Fraction of B=receiver cells 4. Switch Rate ON; 5. Switch Rate OFF" <<endl;
    srand(time(NULL));
    ...
    for(int s=0;s<Ncells;s++){
        if(rand()*1.0/RAND_MAX<fB)Type[s]=1; // B cell = receiver -> can become C (Sticky)
        else Type[s]=0; // A cell = sender (remains A, non sticky)
    }
    ...
    ////COUNT CELLS
    int nB=0; int nA=0; int nC=0;
    for(int s=0;s<Ncells;s++){
        if(Type[s]==0)nA++;
        if(Type[s]==1)nB++;
        if(Type[s]==2)nC++;
    }
    cout << "#CELLS: A=" << nA << ";  B="<<nB << "; C="<<nC<<endl;
    ...
    for(t=1;t<Tmax;t++){
        ...
        r_AA = agg_AA/(agg_AA + agg_AB_BA + agg_AC_CA + agg_BB + agg_BC_CB + agg_CC);
        r_BB = agg_BB/(agg_AA + agg_AB_BA + agg_AC_CA + agg_BB + agg_BC_CB + agg_CC);
        r_CC = agg_CC/(agg_AA + agg_AB_BA + agg_AC_CA + agg_BB + agg_BC_CB + agg_CC);
        r_AB = agg_AB_BA/(agg_AA + agg_AB_BA + agg_AC_CA + agg_BB + agg_BC_CB + agg_CC);
        r_AC = agg_AC_CA/(agg_AA + agg_AB_BA + agg_AC_CA + agg_BB + agg_BC_CB + agg_CC);
        r_BC = agg_BC_CB/(agg_AA + agg_AB_BA + agg_AC_CA + agg_BB + agg_BC_CB + agg_CC);
        
        
        cout << r_AA << " " << r_BB << " " << r_CC <<" " << r_AB << " " << r_AC << " "<< r_BC << endl;
